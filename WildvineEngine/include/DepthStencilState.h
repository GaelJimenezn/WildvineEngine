 ...
 * @brief Encapsula un @c ID3D11DepthStencilState y su ciclo de vida para la etapa
 * Output-Merger.
 *
 * Administra la creación, configuración, aplicación y destrucción de un estado de
 * profundidad/esténcil en Direct3D 11. Permite activar/desactivar pruebas de profundidad
 * y
...
public:
     ...
     * concreta (función de comparación, máscaras, operaciones de esténcil, etc.) debe
     * definirse
     * en la implementación de este método, condicionada por @p enableDepth y @p
     * enableStencil.
     *
     ...
     * @param enableStencil  Habilita el buffer de esténcil y sus pruebas (por defecto @c
     * false).
     ...
     *
     * Método placeholder para futuros cambios dinámicos del descriptor de
     * profundidad/esténcil
    ...
    /**
     * @brief Aplica el estado de profundidad/esténcil al contexto
     * (OMSetDepthStencilState).
     *
     * Vincula el @c ID3D11DepthStencilState al @c DeviceContext. Puede además restablecer
     * el estado
     * a @c nullptr si @p reset es @c true.
     *
     * @param deviceContext  Contexto donde se aplicará el estado.
     * @param stencilRef     Referencia de esténcil usada por las operaciones de esténcil
     * (por defecto 0).
     * @param reset          Si es @c true, desvincula el estado después de aplicarlo
     * (setea @c nullptr).
     *
    ...
    /**
     * @brief Libera el recurso @c ID3D11DepthStencilState y deja la instancia en estado
     * no inicializado.
     *
