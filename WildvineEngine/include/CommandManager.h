 ...
 * @brief Sistema de Undo/Redo basado en el patrÃ³n Command.
 * @ingroup core
 *
 * Proporciona la interfaz ICommand y el gestor CommandManager que mantiene
 * dos pilas (undo/redo) con profundidad mÃ¡xima configurable.
 */
#pragma once
#include <vector>
#include <memory>
 ...
 *
 * Toda acciÃ³n que deba poder deshacerse debe implementar esta interfaz.
...
public:
  ...
  /** @brief Deshace la acciÃ³n realizada por este comando. */
  virtual void undo() = 0;

  /** @brief Re-aplica la acciÃ³n despuÃ©s de un undo. */
  virtual void redo() = 0;
 ...
 * @brief Gestor de historial de Undo/Redo genÃ©rico e independiente del motor.
 *
 * Mantiene una pila de comandos undo y otra de redo con profundidad mÃ¡xima
 * configurable (por defecto 100 entradas). Al ejecutar push() la pila
 * de redo se limpia, tal como es convenciÃ³n en editores tipo Unreal.
 */
class CommandManager {
public:
  /**
   * @brief AÃ±ade un comando al historial y limpia la pila de redo.
   *
   * Si la pila supera maxDepth entradas, las mÃ¡s antiguas se eliminan.
   *
  ...
  /**
   * @brief Deshace el Ãºltimo comando ejecutado.
   *
  ...
  /**
   * @brief Re-aplica el Ãºltimo comando deshecho.
   *
  ...
  /** @brief VacÃ­a ambas pilas (undo y redo). */
  void
  clear() {
    m_undo.clear();
    m_redo.clear();
  ...
  /**
   * @brief Devuelve el nÃºmero de comandos en la pila de undo.
   * @return NÃºmero de acciones deshacibles.
   */
  size_t
  undoCount() const {
    return m_undo.size();
...
private:
  ...
  size_t m_maxDepth = 100; /**< @brief Profundidad mÃ¡xima del historial de undo. */
};
