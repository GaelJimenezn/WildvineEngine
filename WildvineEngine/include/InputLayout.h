...
/**
 * @class InputLayout
 * @brief Encapsula un @c ID3D11InputLayout que describe el formato de los vértices para
 * el pipeline de entrada.
 *
 * Un Input Layout en Direct3D 11 define cómo se interpretan los datos de un Vertex Buffer
 * (posición, normales, UVs, colores, etc.) y cómo se asignan a las entradas de un Vertex
 * Shader.
 *
...
public:
    ...
    /**
     * @brief Inicializa el Input Layout a partir de una descripción y bytecode de Vertex
     * Shader.
     *
     ...
     * (semánticas, formato, offset, etc.).
     * @param VertexShaderData  Bytecode compilado del Vertex Shader que contiene la firma
     * de entrada.
    ...
    /**
     * @brief Libera el recurso @c ID3D11InputLayout y deja la instancia en estado no
     * inicializado.
     *
