...
public:
  ...
  void bind(DeviceContext &deviceContext);
  ...
  getSRV() const {
    return m_colorSRV.m_textureFromImg;
  }

  /**
   * @brief Obtiene el ancho del viewport.
   * @return Ancho en pÃ­xeles.
   */
  unsigned
  ...
  /**
   * @brief Verifica si los recursos son vÃ¡lidos.
   * @return true si todos los recursos estÃ¡n correctamente inicializados.
   */
  bool
  isValid() const {
    return m_colorTexture.m_texture != nullptr &&
           m_colorSRV.m_textureFromImg != nullptr && m_depthTexture.m_texture != nullptr;
  }
