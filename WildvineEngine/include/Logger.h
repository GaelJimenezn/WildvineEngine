 ...
 * escribir en Ã©l sin bloquear el render thread gracias al mutex interno.
 */
#pragma once
#include <string>
#include <vector>
...
/**
 * @struct LogEntry
 * @brief Representa una Ãºnica entrada en el buffer de log.
 */
struct LogEntry {
...
public:
   ...
   * @return Referencia a la Ãºnica instancia del Logger.
   */
  static Logger &
  get() {
   ...
   *
   * Si el buffer supera 8000 entradas, descarta las 2000 mÃ¡s antiguas
   ...
   */
  void
  add(LogLevel level, const std::string &msg) {
    if (!shouldStore(level, msg)) {
      return;
    }
    /** @brief Declara o ejecuta lock. */
   ...
   */
  void
  addW(LogLevel level, const std::wstring &wmsg) {
  ...
  /**
   * @brief VacÃ­a completamente el buffer de log.
   */
  void
  clear() {
    /** @brief Declara o ejecuta lock. */
   ...
   * luego false hasta que se agregan mÃ¡s. Ãštil para refrescar la UI solo
   * cuando hay cambios reales.
   *
   * @return true si el buffer fue modificado desde la Ãºltima llamada.
   */
  bool
  consumeDirty() {
    /** @brief Declara o ejecuta lock. */
...
private:
  ...
  /**
     * @brief Decide que mensajes deben aparecer en la consola del editor.
     *

   * * La salida de Visual Studio puede recibir mensajes detallados de recursos,
     *
   * pero la consola visible del editor solo conserva errores y eventos utiles
     * para
   * el flujo del usuario: importacion, modelos, texturas, escena y prefabs.
     */
  static bool
  shouldStore(LogLevel level, const std::string &msg) {
    if (level == LogLevel::Error || level == LogLevel::Warning) {
      return true;
    }

    const std::string lower = toLower(msg);
    return contains(lower, "import") || contains(lower, "model") ||
           contains(lower, "modelo") || contains(lower, "texture") ||
           contains(lower, "textura") || contains(lower, "thumbnail") ||
           contains(lower, "scene") || contains(lower, "escena") ||
           contains(lower, "prefab") || contains(lower, "binary cache") ||
           contains(lower, "cache");
  }

  /** @brief Devuelve true si una cadena contiene otra. */
  static bool
  contains(const std::string &value, const std::string &needle) {
    return value.find(needle) != std::string::npos;
  }

  /** @brief Convierte texto ASCII a minusculas para filtrar mensajes. */
  static std::string
  ...
  /** @brief Declara o ejecuta narrow. */
  static std::string
  narrow(const std::wstring &w) {
    std::string s;
    s.reserve(w.size());
    for (wchar_t c : w) {
