...
ResolveViewportRTV(EditorViewportPass &pass) {
  ...
  RenderTargetViewAccess *rtvAccess =
      reinterpret_cast<RenderTargetViewAccess *>(&access->m_rtv);
...
writeLightToFrameBuffer(CBPerFrame &buffer, int lightIndex, const LightData &light) {
  ...
  buffer.LightPositionsRanges[lightIndex] =
      XMFLOAT4(light.position.x, light.position.y, light.position.z, range);
  buffer.LightColorsTypes[lightIndex] =
      XMFLOAT4(lightColor.x,
               lightColor.y,
               lightColor.z,
  ...
  buffer.LightDirectionsIntensities[lightIndex] =
      XMFLOAT4(light.direction.x, light.direction.y, light.direction.z, wChannel);
}
} // namespace
...
DeferredRenderer::init(Device &device) {
  ...
  hr = m_transparentDepthStencil.init(
      device, true, D3D11_DEPTH_WRITE_MASK_ZERO, D3D11_COMPARISON_LESS_EQUAL);
  if (FAILED(hr)) {
    return hr;
  }

  hr = m_disabledDepthStencil.init(
      device, false, D3D11_DEPTH_WRITE_MASK_ZERO, D3D11_COMPARISON_ALWAYS);
  if (FAILED(hr)) {
    return hr;
  }

  hr = m_shadowDepthStencil.init(
      device, true, D3D11_DEPTH_WRITE_MASK_ALL, D3D11_COMPARISON_LESS);
  if (FAILED(hr)) {
    return hr;
  }
...
DeferredRenderer::updatePerFrame(const Camera &camera,
  ...
  // --- Fill all lights into the per-frame buffer ---
  {
    ...
    if (!scene.directionalLights.empty()) {
      const LightData &mainLight =
          primaryShadowLight ? *primaryShadowLight : scene.directionalLights[0];
...
DeferredRenderer::updateLightMatrices(const Camera &camera, const RenderScene &scene) {
  ...
  XMVECTOR lightDirVec =
      XMVector3Normalize(XMVectorSet(lightDir.x, lightDir.y, lightDir.z, 0.0f));
...
DeferredRenderer::renderSceneToTarget(DeviceContext &deviceContext,
  ...
  bindFinalTarget(
      deviceContext, ResolveViewportRTV(targetPass), ResolveViewportDSV(targetPass));
...
void
DeferredRenderer::bindGBufferTargets(DeviceContext &deviceContext,
...
void
DeferredRenderer::renderGeometryObject(DeviceContext &deviceContext,
  ...
  for (Submesh &submesh : submeshes) {
    ...
    m_cbPerMaterial.AlphaCutoff =
        material->getDomain() == MaterialDomain::Masked ? params.alphaCutoff : 0.0f;
    m_perMaterialBuffer.update(
        deviceContext, nullptr, 0, nullptr, &m_cbPerMaterial, 0, 0);
...
DeferredRenderer::renderLightingPass(DeviceContext &deviceContext) {
  ...
  m_lightingDebugData.DebugViewMode =
      m_shadowFactorDebugEnabled ? 1 : m_deferredDebugViewMode;
  m_lightingDebugData.ShadowStrength = 1.0f;
  m_lightingDebugBuffer.update(
      deviceContext, nullptr, 0, nullptr, &m_lightingDebugData, 0, 0);
...
DeferredRenderer::renderTransparentPass(DeviceContext &deviceContext) {
  ...
  for (const RenderObject *object : m_transparentQueue) {
    ...
    Material *material =
        object->materialInstance ? object->materialInstance->getMaterial() : nullptr;
  ...
  for (Submesh &submesh : submeshes) {
    ...
    if (passType == RenderPassType::Transparent) {
      deviceContext.OMSetBlendState(
          resolveBlendState(material), m_blendFactor, 0xffffffff);
...
DeferredRenderer::renderShadowPass(DeviceContext &deviceContext) {
  ...
  deviceContext.ClearDepthStencilView(
      m_shadowDSV.m_depthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
...
void
DeferredRenderer::renderShadowObject(DeviceContext &deviceContext,
                                     const RenderObject &object) {
  if (!object.mesh) {
    return;
  }
...
DeferredRenderer::createShadowResources(Device &device) {
  HRESULT hr =
      m_shadowDepthTexture.init(device,
                                m_shadowMapSize,
                                m_shadowMapSize,
  ...
  hr = m_shadowDepthSRV.init(
      device, m_shadowDepthTexture, DXGI_FORMAT_R24_UNORM_X8_TYPELESS);
  if (FAILED(hr)) {
    return hr;
  }
...
HRESULT
DeferredRenderer::createGBufferResources(Device &device,
                                         unsigned int width,
                                         unsigned int height) {
  m_gBufferEmissiveAlphaRTV.destroy();
  m_gBufferEmissiveAlphaSRV.destroy();
...
DeferredRenderer::createLightingResources(Device &device) {
  ...
  return m_fullscreenRasterizer.init(
      device, D3D11_FILL_SOLID, D3D11_CULL_NONE, false, false);
}

HRESULT
DeferredRenderer::createFullScreenQuad(Device &device) {
  MeshComponent mesh;
  mesh.m_vertex = {{EU::Vector3(-1.0f, -1.0f, 0.0f),
                    EU::Vector3(),
                    EU::Vector3(),
                    EU::Vector3(),
                    EU::Vector2(0.0f, 1.0f)},
                   {EU::Vector3(-1.0f, 1.0f, 0.0f),
                    EU::Vector3(),
                    EU::Vector3(),
                    EU::Vector3(),
