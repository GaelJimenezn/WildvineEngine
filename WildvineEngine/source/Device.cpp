...
//
// La función `destroy` se encarga de liberar el objeto principal de Direct3D, el
// ID3D11Device. Esto es crucial para liberar todos los recursos de la GPU cuando la
// aplicación termina.
//
void
Device::destroy() {
...
// Un Render Target View es una "vista" de un recurso (como una textura) que se puede usar
// como destino para renderizar.
//
HRESULT
Device::CreateRenderTargetView(ID3D11Resource *pResource,
  ...
  if (SUCCEEDED(hr)) {
    MESSAGE(
        "Device", "CreateRenderTargetView", "Render Target View created successfully!");
  } else {
    ERROR(
        "Device",
...
// Este buffer es esencial para el Z-Buffering, que ayuda a determinar qué polígonos son
// visibles.
//
HRESULT
Device::CreateDepthStencilView(ID3D11Resource *pResource,
  ...
  if (SUCCEEDED(hr)) {
    MESSAGE(
        "Device", "CreateDepthStencilView", "Depth Stencil View created successfully!");
  } else {
    ERROR(
        "Device",
...
// Esto le dice a la tarjeta gráfica cómo interpretar la información de los vértices
// (posición, color, etc.).
//
HRESULT
...
// Los estados de muestreo controlan cómo se lee la información de las texturas (por
// ejemplo, cómo se filtra).
//
HRESULT
...
// Los buffers se usan para almacenar vértices, índices, constantes, y otros datos que
// necesita la GPU.
//
HRESULT
