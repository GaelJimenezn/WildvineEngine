...
void
Octree::build(const std::vector<RenderObject> &objects) {
  m_root.reset();
  if (objects.empty())
    return;
  ...
  for (const RenderObject &object : objects)
    insert(*m_root, object, 0);
}

void
Octree::queryVisible(const Camera &camera,
...
bool
Octree::isVisible(const EU::Vector3 &minimum,
  ...
  for (int corner = 0; corner < 8; ++corner) {
    XMVECTOR point = XMVectorSet((corner & 1) ? maximum.x : minimum.x,
  ...
  return !(outsideLeft || outsideRight || outsideBottom || outsideTop || outsideNear ||
           outsideFar);
}

bool
Octree::fitsInside(const RenderObject &object, const Node &node) {
  return object.boundsMin.x >= node.minimum.x && object.boundsMin.y >= node.minimum.y &&
         object.boundsMin.z >= node.minimum.z && object.boundsMax.x <= node.maximum.x &&
         object.boundsMax.y <= node.maximum.y && object.boundsMax.z <= node.maximum.z;
}

void
Octree::subdivide(Node &node) {
  const EU::Vector3 center((node.minimum.x + node.maximum.x) * 0.5f,
                           (node.minimum.y + node.maximum.y) * 0.5f,
                           (node.minimum.z + node.maximum.z) * 0.5f);
  for (int index = 0; index < 8; ++index) {
    std::unique_ptr<Node> child = std::make_unique<Node>();
    child->minimum = EU::Vector3((index & 1) ? center.x : node.minimum.x,
                                 (index & 2) ? center.y : node.minimum.y,
                                 (index & 4) ? center.z : node.minimum.z);
    child->maximum = EU::Vector3((index & 1) ? node.maximum.x : center.x,
...
void
Octree::insert(Node &node, const RenderObject &object, unsigned int depth) {
  if (depth >= kMaxDepth) {
  ...
  if (!node.children[0] && node.objects.size() >= kNodeCapacity) {
    ...
    for (const RenderObject &current : pending)
      insert(node, current, depth);
  }
  if (node.children[0]) {
...
void
Octree::queryNode(const Node &node,
                  const XMMATRIX &viewProjection,
                  std::vector<RenderObject> &output,
                  CullingStats &stats) {
  ++stats.visitedNodes;
  ...
  for (const std::unique_ptr<Node> &child : node.children) {
    if (child)
      queryNode(*child, viewProjection, output, stats);
  }
}

unsigned int
Octree::countObjects(const Node &node) {
