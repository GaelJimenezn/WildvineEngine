...
SceneGraph::detach(Entity *child) {
  ...
  if (!parent)
    return true; // ya estaba root

  auto hp = parent->getComponent<HierarchyComponent>();
  if (hp)
    hp->removeChild(child);
  ...
  for (Entity *entity : m_entities) {
    ...
    if (lightComponent) {
      ...
      case LightType::Point:
        outScene.pointLights.push_back(light);
        break;
      case LightType::Spot:
        outScene.spotLights.push_back(light);
        break;
      case LightType::Rect:
        outScene.rectLights.push_back(light);
        break;
      default:
    ...
    EU::Vector3 objectPos =
        EU::Vector3(worldMatrix._41, worldMatrix._42, worldMatrix._43);
