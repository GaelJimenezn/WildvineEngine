...
RenderTargetView::init(Device &device, Texture &backBuffer, DXGI_FORMAT Format) {
  // Se verifica que el dispositivo y la textura no sean nulos y que el formato sea
  // válido.
  if (!device.m_device) {
  ...
  // Se crea la vista de destino de renderizado llamando a la función del dispositivo de
  // Direct3D.
  HRESULT hr = device.m_device->CreateRenderTargetView(
      backBuffer.m_texture, &desc, &m_renderTargetView);
...
//
// La segunda función `init` es una versión más general que inicializa el Render Target
// View con una dimensión y un formato dados, no solo para back buffers.
//
HRESULT
RenderTargetView::init(Device &device,
  ...
  HRESULT hr = device.m_device->CreateRenderTargetView(
      inTex.m_texture, &desc, &m_renderTargetView);
...
RenderTargetView::render(DeviceContext &deviceContext,
  ...
  deviceContext.m_deviceContext->OMSetRenderTargets(
      numViews, &m_renderTargetView, depthStencilView.m_depthStencilView);
}

//
// La segunda función `render` es una versión simplificada que solo asigna la vista de
// destino de renderizado, sin un buffer de profundidad/plantilla.
//
void
RenderTargetView::render(DeviceContext &deviceContext, unsigned int numViews) {
  ...
  // Se configura la vista de destino de renderizado, pasando nullptr para el buffer de
  // profundidad.
  deviceContext.m_deviceContext->OMSetRenderTargets(
      numViews, &m_renderTargetView, nullptr);
}
