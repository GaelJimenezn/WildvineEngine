...
bool
IsModelFile(const fs::path &path) {
  ...
  return extension == ".fbx" || extension == ".obj" || extension == ".glb" ||
         extension == ".gltf";
}

bool
IsImageFile(const fs::path &path) {
  std::string extension = path.extension().string();
  for (char &character : extension) {
    if (character >= 'A' && character <= 'Z') {
      character = static_cast<char>(character + 32);
...
bool
IsAudioFile(const fs::path &path) {
  std::string extension = path.extension().string();
  for (char &character : extension) {
    if (character >= 'A' && character <= 'Z') {
      character = static_cast<char>(character + 32);
...
bool
IsPlayableAudioFile(const fs::path &path) {
  std::string extension = path.extension().string();
  for (char &character : extension) {
    if (character >= 'A' && character <= 'Z') {
      character = static_cast<char>(character + 32);
...
bool
ConvertAudioToWave(const fs::path &sourcePath, const fs::path &outputPath) {
  ...
  if (FAILED(MFCreateSourceReaderFromURL(
          sourcePath.c_str(), nullptr, reader.GetAddressOf())))
    return false;
  ...
  const UINT32 channels =
      MFGetAttributeUINT32(currentType.Get(), MF_MT_AUDIO_NUM_CHANNELS, 0);
  const UINT32 sampleRate =
      MFGetAttributeUINT32(currentType.Get(), MF_MT_AUDIO_SAMPLES_PER_SECOND, 0);
  const UINT32 bitsPerSample =
      MFGetAttributeUINT32(currentType.Get(), MF_MT_AUDIO_BITS_PER_SAMPLE, 0);
...
std::string
ToContentPath(const fs::path &path) {
  return path.generic_string();
}

bool
ImportModelWithTextures(const fs::path &sourcePath) {
  const fs::path modelDirectory = fs::path("Assets") / "Models" / sourcePath.stem();
  ...
  fs::copy_file(
      sourcePath, destinationModel, fs::copy_options::overwrite_existing, error);
  if (error)
    return false;
  ...
  for (const fs::directory_entry &entry :
    ...
    if (error)
      return false;
    fs::copy_file(
        entry.path(), destinationTexture, fs::copy_options::overwrite_existing, error);
    if (error)
      return false;
  }
...
void
DrawDebugTextureEntry(const DebugTextureItem &item,
                      int index,
                      int &selectedView,
...
void
DrawInspectorPill(const char *text, const ImVec4 &color) {
...
bool
BeginInspectorSection(const char *label, bool defaultOpen = true) {
...
bool
BeginInspectorPropertyTable(const char *id, float firstColumnWidth = 132.0f) {
  if (!ImGui::BeginTable(
          id, 2, ImGuiTableFlags_SizingStretchProp | ImGuiTableFlags_BordersInnerV)) {
    return false;
  }
...
void
DrawPropertyLabel(const char *label) {
  ImGui::TableNextRow();
...
void
DrawPropertyValueText(const char *label, const char *value) {
  ImGui::TableNextRow();
  ImGui::TableSetColumnIndex(0);
  ImGui::AlignTextToFramePadding();
  ImGui::TextDisabled("%s", label);
...
void
DrawPropertyValueBool(const char *label, bool value) {
  DrawPropertyValueText(label, value ? "Yes" : "No");
}

void
DrawPropertyToggle(const char *label, const char *id, bool *value) {
  ImGui::TableNextRow();
  ImGui::TableSetColumnIndex(0);
  ImGui::AlignTextToFramePadding();
  ImGui::TextDisabled("%s", label);
...
const char *
GetActorTypeLabel(EU::TSharedPointer<Actor> actor) {
  if (actor.isNull()) {
    return "Actor";
  }
...
ImVec4
GetActorTypeColor(EU::TSharedPointer<Actor> actor) {
  if (actor.isNull()) {
...
void
DrawInspectorComponentChips(bool hasTransform, bool hasMeshRenderer, bool hasLight) {
  if (hasTransform) {
...
const char *
GetLightTypeLabel(LightType type) {
  switch (type) {
...
const char *
GetMaterialDomainLabel(MaterialDomain domain) {
  switch (domain) {
...
const char *
GetBlendModeLabel(BlendMode blendMode) {
  switch (blendMode) {
...
GUI::vec3Control(const std::string &label,
  ...
  if (!ImGui::BeginTable(("##Vec3Table" + label).c_str(),
                         2,
  ...
  const float dragWidth =
      (availableWidth - (buttonSize.x * 3.0f) - (spacing * 5.0f)) / 3.0f;
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("Tools")) {
      if (ImGui::MenuItem("Options")) {
        // AcciÃ³n para "Options"
      }
      if (ImGui::MenuItem("Settings")) {
        // AcciÃ³n para "Settings"
      }
      ImGui::EndMenu();
    }
...
GUI::closeApp() {
  ...
  if (ImGui::BeginPopupModal("Exit?", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (ImGui::Button("OK", ImVec2(120, 0))) {
      exit(0); // Salir de la aplicaciÃ³n
...
GUI::inspectorGeneral(EU::TSharedPointer<Actor> actor) {
  ...
  if (actor.isNull()) {
    ...
    ImGui::TextWrapped("Select an actor in the Hierarchy to inspect transforms, "
                       "materials, lights and renderer data.");
    ImGui::End();
    return;
  }
  ...
  ImVec2 headerMax = ImVec2(headerMin.x + ImGui::GetWindowSize().x,
                            headerMin.y + ImGui::GetWindowSize().y);
  drawList->AddRectFilled(headerMin,
                          ImVec2(headerMax.x, headerMin.y + 4.0f),
                          AccentU32(accentColor),
                          6.0f,
  ...
  if (hasMeshRenderer) {
    const std::vector<MaterialInstance *> &materialInstances =
        meshRenderer->getMaterialInstances();
        ...
        if (ImGui::TreeNodeEx(header.c_str(),
          ...
          if (material) {
            if (BeginInspectorPropertyTable(
              ...
              DrawPropertyValueText("Domain",
                                    GetMaterialDomainLabel(material->getDomain()));
              if (material->getDomain() == MaterialDomain::Transparent) {
                DrawPropertyValueText("Blend",
                                      GetBlendModeLabel(material->getBlendMode()));
              }
              ImGui::EndTable();
            }
            ...
            if (BeginInspectorPropertyTable(
              ...
              if (ImGui::Combo(("##Domain" + std::to_string(i)).c_str(),
                               &currentDomain,
                               kMaterialDomains,
                               IM_ARRAYSIZE(kMaterialDomains))) {
              ...
              if (material->getDomain() == MaterialDomain::Transparent) {
                ...
                if (ImGui::Combo(("##BlendMode" + std::to_string(i)).c_str(),
                                 &currentBlendMode,
                                 kBlendModes,
                                 IM_ARRAYSIZE(kBlendModes))) {
                  material->setBlendMode(static_cast<BlendMode>(currentBlendMode));
                }
              }

              DrawPropertyLabel("Base Color");
              ImGui::ColorEdit4(("##BaseColor" + std::to_string(i)).c_str(),
                                &params.baseColor.x);
              ...
              ImGui::SliderFloat(
                  ("##AO" + std::to_string(i)).c_str(), &params.ao, 0.0f, 1.0f);
              DrawPropertyLabel("Normal Scale");
              ImGui::SliderFloat(("##NormalScale" + std::to_string(i)).c_str(),
                                 &params.normalScale,
                                 0.0f,
                                 2.0f);
              if (materialInstance->getEmissive()) {
                DrawPropertyLabel("Emissive Strength");
                ImGui::SliderFloat(("##EmissiveStrength" + std::to_string(i)).c_str(),
              ...
              if (material->getDomain() == MaterialDomain::Masked) {
                ...
                ImGui::SliderFloat(("##AlphaCutoff" + std::to_string(i)).c_str(),
                                   &params.alphaCutoff,
                                   0.0f,
                                   1.0f);
              }
  ...
  if (!audioSource.isNull() && BeginInspectorSection("Audio Source")) {
    char audioPath[260] = {};
    strncpy_s(audioPath, audioSource->getAudioPath().c_str(), _TRUNCATE);
    if (ImGui::InputText("WAV Path", audioPath, IM_ARRAYSIZE(audioPath))) {
      audioSource->setAudioPath(audioPath);
    }

    if (ImGui::BeginDragDropTarget()) {
      const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("DND_AUDIO_PATH");
      if (payload) {
    ...
    if (spatial) {
      ...
      if (ImGui::DragFloat("Min Distance", &minDistance, 0.1f, 0.0f, 1000.0f, "%.1f")) {
        audioSource->setAttenuation(minDistance, maxDistance);
      }
      if (ImGui::DragFloat("Max Distance", &maxDistance, 0.1f, 0.1f, 1000.0f, "%.1f")) {
        audioSource->setAttenuation(minDistance, maxDistance);
      }
    }
  ...
  if (!particleEmitter.isNull() && BeginInspectorSection("Particle Emitter")) {
    ...
    ImGui::ColorEdit3("Start Color", &particleEmitter->startColor().x);
    ImGui::ColorEdit3("End Color", &particleEmitter->endColor().x);
  }
  ImGui::End();
}
...
GUI::outliner(const std::vector<EU::TSharedPointer<Actor>> &actors) {
  ...
  for (int i = 0; i < static_cast<int>(actors.size()); ++i) {
    ...
    auto meshRenderer = actor ? actor->getComponent<MeshRendererComponent>()
                              : EU::TSharedPointer<MeshRendererComponent>();
    auto lightComponent = actor ? actor->getComponent<LightComponent>()
                                : EU::TSharedPointer<LightComponent>();
    const bool hasMeshRenderer = !meshRenderer.isNull();
    const bool hasLightComponent = !lightComponent.isNull();
    ...
    ImDrawList *drawList = ImGui::GetWindowDrawList();
    drawList->AddText(ImVec2(min.x + 12.0f, min.y + 6.0f),
                      ImGui::GetColorU32(ImGuiCol_Text),
                      actorName.c_str());
    drawList->AddText(
        ImVec2(min.x + 12.0f, min.y + 23.0f), AccentU32(actorTypeColor), actorTypeLabel);

    float badgeX = max.x - 84.0f;
    if (hasMeshRenderer) {
      drawList->AddRectFilled(ImVec2(badgeX, min.y + 12.0f),
                              ImVec2(badgeX + 28.0f, min.y + 30.0f),
                              IM_COL32(68, 118, 180, 180),
                              6.0f);
      drawList->AddText(
          ImVec2(badgeX + 9.0f, min.y + 14.0f), IM_COL32(240, 244, 255, 255), "M");
      badgeX += 40.0f;
    }
    if (hasLightComponent) {
      drawList->AddRectFilled(ImVec2(badgeX, min.y + 12.0f),
                              ImVec2(badgeX + 28.0f, min.y + 30.0f),
                              IM_COL32(180, 142, 52, 180),
                              6.0f);
      drawList->AddText(
          ImVec2(badgeX + 9.0f, min.y + 14.0f), IM_COL32(255, 248, 232, 255), "L");
    }
...
GUI::editTransform(Camera &cam, Window &window, EU::TSharedPointer<Actor> actor) {
  ...
  auto audioSource = actor->getComponent<AudioSourceComponent>();
  if (mEditingAudioRange && !audioSource.isNull() && audioSource->isSpatial()) {
    ...
    ImGuizmo::SetID(1);
    ImGuizmo::Manipulate(vArr, pArr, ImGuizmo::SCALE, ImGuizmo::LOCAL, rangeMatrix);
    ...
    if (m_isUsingGizmo) {
      ...
      const float newMaxDistance =
          fmaxf(fabsf(newScale[0]), fmaxf(fabsf(newScale[1]), fabsf(newScale[2])));
      audioSource->setAttenuation(audioSource->getMinDistance(), newMaxDistance);
    }
    return;
  }
...
GUI::drawGizmoToolbar() {
  ...
  if (ImGui::Begin("GizmoToolBar", nullptr, window_flags)) {
    auto buttonMode =
        [&](const char *label, ImGuizmo::OPERATION op, const char *shortcut) {
    ...
    const bool worldLocalSupported =
        (mGizmoEnabled && mCurrentGizmoOperation != ImGuizmo::SCALE);
    ...
    if (ImGui::Button(mCurrentGizmoMode == ImGuizmo::WORLD ? "Global" : "Local"))
      mCurrentGizmoMode =
          (mCurrentGizmoMode == ImGuizmo::WORLD) ? ImGuizmo::LOCAL : ImGuizmo::WORLD;
    if (!worldLocalSupported) {
      ImGui::PopStyleVar();
      ImGui::PopItemFlag();
      if (ImGui::IsItemHovered()) {
...
GUI::drawStudioTopRibbon() {
    ...
    if (ImGui::BeginMenuBar()) {
      ...
      if (ImGui::BeginMenu("Create")) {
        ...
        if (ImGui::MenuItem("Particle Emitter"))
          m_requestCreateParticleEmitter = true;
      ...
      if (ImGui::BeginMenu("Renderer")) {
        ...
        ImGui::MenuItem("Material SRV Inspector", nullptr, &m_showMaterialSRVDebug);
        ImGui::MenuItem(
            "Visualize Shadow Factor", nullptr, &m_visualizeDeferredShadowFactor);
        ImGui::EndMenu();
      }
  ...
  if (ImGui::Begin("##CompactEditorToolbar", nullptr, toolbarFlags)) {
    ...
    if (!m_isRuntimePlaying) {
      ...
      ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.60f, 0.12f, 0.14f, 1.0f));
    ...
    if (ImGui::BeginPopup("##CreateActorMenu")) {
      ...
      ImGui::InputTextWithHint("##CreateSearch",
                               "Start typing to search",
                               createSearchBuffer,
                               IM_ARRAYSIZE(createSearchBuffer));
      ImGui::Separator();
...
GUI::drawViewportPanel(ID3D11ShaderResourceView *viewportSRV) {
  ...
  if (ImGui::Begin(m_isRuntimePlaying ? "Gameplay" : "DefaultScene", nullptr, flags)) {
    ...
    if (!m_isRuntimePlaying && ImGui::BeginMenuBar()) {
      ...
      if (ImGui::BeginMenu("Menu")) {
        ...
        if (ImGui::MenuItem("Translate Mode",
                            "W",
                            mGizmoEnabled &&
        ...
        ImGui::Separator();
        if (ImGui::MenuItem("Coordinate System",
                            mCurrentGizmoMode == ImGuizmo::WORLD ? "World" : "Local")) {
          mCurrentGizmoMode =
              (mCurrentGizmoMode == ImGuizmo::WORLD) ? ImGuizmo::LOCAL : ImGuizmo::WORLD;
        }
        ImGui::Separator();
    ...
    if (viewportSRV) {
      ...
      ImGui::SetCursorScreenPos(
          ImVec2(panelMin.x + imageOffset.x, panelMin.y + imageOffset.y));
      ...
      drawList->AddText(ImVec2(itemMin.x + 12.0f, itemMin.y + 12.0f),
    ...
    if (ImGui::BeginDragDropTarget()) {
      ...
      if (const ImGuiPayload *payload =
              ImGui::AcceptDragDropPayload("DND_TEXTURE_PATH")) {
        const char *path = (const char *)payload->Data;
...
GUI::drawRenderDebugPanel(ID3D11ShaderResourceView *preShadowSRV,
  ...
  DebugTextureItem items[] = {{"Scene Final", "Viewport color target", finalViewportSRV},
  ...
  for (int i = 0; i < IM_ARRAYSIZE(items); ++i) {
    ...
    if (ImGui::Selectable("##RenderPassRow",
                          selected,
    ...
    ImDrawList *drawList = ImGui::GetWindowDrawList();
    drawList->AddText(ImVec2(rowMin.x + 8.0f, rowMin.y + 7.0f),
                      ImGui::GetColorU32(ImGuiCol_Text),
                      items[i].label);
    drawList->AddText(ImVec2(rowMin.x + 8.0f, rowMin.y + 25.0f),
    ...
    if (items[i].srv) {
      ...
      ImGui::Image((ImTextureID)items[i].srv,
                   ImVec2(thumbMax.x - thumbMin.x, thumbMax.y - thumbMin.y));
    } else {
      ...
      drawList->AddText(ImVec2(thumbMin.x + 8.0f, thumbMin.y + 8.0f),
                        ImGui::GetColorU32(ImGuiCol_TextDisabled),
                        "Unavailable");
    }
  ...
  if (items[selectedView].srv) {
    ...
    drawList->AddText(ImVec2(min.x + 16.0f, min.y + 16.0f),
                      ImGui::GetColorU32(ImGuiCol_TextDisabled),
                      "No SRV bound for this pass");
  }
  ImGui::EndChild();
...
GUI::drawGBufferDebugPanel(ID3D11ShaderResourceView *albedoMetallicSRV,
  ...
  ImGui::SameLine();
  ImGui::Text("%s",
              hasAnyTexture ? "PBR fallback from loaded SRVs"
                            : "Deferred attachments unavailable");
  ...
  if (items[selectedView].srv) {
    ...
    drawList->AddText(
        ...
        ImGui::GetColorU32(ImGuiCol_TextDisabled),
        "Open a loaded material or deferred renderer output to populate this preview");
  }
  ImGui::EndChild();
...
void
GUI::drawMaterialSRVDebugPanel(ID3D11ShaderResourceView *albedoSRV,
  ...
  if (items[selectedView].srv) {
    ...
    drawList->AddText(ImVec2(min.x + 16.0f, min.y + 16.0f),
                      ImGui::GetColorU32(ImGuiCol_TextDisabled),
                      "This material slot has no shader resource view");
  }
  ImGui::EndChild();
...
GUI::drawEditorDockspace() {
  ...
  if (!s_defaultLayoutBuilt || ImGui::DockBuilderGetNode(dockspace_id) == nullptr) {
    ...
    ImGuiID dockLeft =
        ImGui::DockBuilderSplitNode(dockMain, ImGuiDir_Left, 0.18f, nullptr, &dockMain);
    ImGuiID dockRightBottom =
        ImGui::DockBuilderSplitNode(dockRight, ImGuiDir_Down, 0.50f, nullptr, &dockRight);

    // Centro: Viewport â€” DefaultScene va al Ãºltimo para quedar activo
    ...
    // Izquierda: Lighting primero, World Outliner al Ãºltimo para quedar activo
...
void
GUI::drawViewportGrid(Camera &cam) {
  if (!m_showGrid)
    return;
  if (m_viewportSize.x < 16.0f || m_viewportSize.y < 16.0f)
    return;
...
void
GUI::drawLightingPanel(float *lightDir, float *lightColor) {
  ...
  ImGui::PopStyleColor(3);
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("Restaura transforms, luz y camara a sus valores originales");
  ImGui::Separator();
...
GUI::drawAudioPanel() {
  ...
  if (ImGui::SliderFloat("Master Volume", &masterVolumePercent, 0.0f, 100.0f, "%.0f%%")) {
  ...
  ImGui::SameLine();
  ImGui::TextDisabled(m_audioPaused ? "All sources paused" : "All sources active");
  ImGui::Spacing();
  ImGui::TextWrapped("Configure each WAV path, source volume, and playback in Details.");
  ImGui::End();
}

void
GUI::drawStatsPanel(float deltaTime,
  ...
  ImGui::Text("Objects: %u visible / %u total", visibleObjects, submittedObjects);
  ImGui::Text("Frustum culled: %u  |  Octree nodes: %u", culledObjects, octreeNodes);
  ImGui::TextDisabled("Viewport: %.0f x %.0f", m_viewportSize.x, m_viewportSize.y);
  ImGui::End();
}

void
GUI::drawConsolePanel() {
  ImGui::Begin("Console");
  if (ImGui::Button("Clear"))
    Logger::get().clear();
  ImGui::SameLine();
  ...
  ImGui::SameLine();
  ImGui::Checkbox("Warning", &m_logShowWarning);
  ImGui::SameLine();
  ...
  for (const LogEntry &e : entries) {
    if (e.level == LogLevel::Info && !m_logShowInfo)
      continue;
    if (e.level == LogLevel::Warning && !m_logShowWarning)
      continue;
    if (e.level == LogLevel::Error && !m_logShowError)
      continue;
    if (!m_logFilter.PassFilter(e.message.c_str()))
      continue;
    ImVec4 col;
    const char *tag;
    switch (e.level) {
    ...
    case LogLevel::Warning:
      col = ImVec4(0.95f, 0.78f, 0.30f, 1.0f);
      tag = "[WARN]  ";
      break;
    default:
      col = ImVec4(0.80f, 0.78f, 0.90f, 1.0f);
      tag = "[INFO]  ";
      break;
    }
...
void
GUI::drawContentBrowser(const std::vector<AssetThumb> &textureThumbs) {
  ImGui::Begin("Content");
  ...
  if (ImGui::Button("Import Content...")) {
    ...
    ofn.lpstrFilter =
        "All "
        "Supported\0*.fbx;*.obj;*.glb;*.gltf;*.png;*.jpg;*.tga;*.dds;*.wav;*.mp3;*."
        "flac\0Models (*.fbx;*.obj;*.glb;*.gltf)\0*.fbx;*.obj;*.glb;*.gltf\0Textures "
        "(*.png;*.jpg;*.tga;*.dds)\0*.png;*.jpg;*.tga;*.dds\0Audio "
        "(*.wav;*.mp3;*.flac)\0*.wav;*.mp3;*.flac\0All\0*.*\0";
    ofn.nFilterIndex = 1;
      ...
      if (IsModelFile(sourcePath)) {
        ...
        if (!error && IsPlayableAudioFile(sourcePath)) {
          fs::copy_file(sourcePath,
                        audioDirectory / sourcePath.filename(),
        ...
        if (!error) {
          fs::copy_file(
              sourcePath, destination, fs::copy_options::overwrite_existing, error);
          imported = !error;
        }
      }
      if (imported)
  ...
  if (ImGui::BeginPopupModal(
          "Create Content Folder", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
  ...
  if (ImGui::BeginTabBar("##ContentTabs")) {
    if (ImGui::BeginTabItem("Models")) {
      ...
      for (const fs::directory_entry &entry :
        ...
        if (!error && entry.is_regular_file() && IsModelFile(entry.path())) {
          models.push_back(ToContentPath(entry.path().lexically_relative(modelsRoot)));
        }
      }
      if (models.empty())
        ImGui::TextDisabled("No hay modelos en Assets/Models");
      const float cell = 90.0f;
      float availW = ImGui::GetContentRegionAvail().x;
      int perRow = (int)(availW / (cell + 10.0f));
      if (perRow < 1)
        perRow = 1;
      int col = 0;
      for (const std::string &m : models) {
        ...
        if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID)) {
          ...
          ImGui::SetDragDropPayload(
              "DND_MODEL_PATH", fullPath.c_str(), fullPath.size() + 1);
          ImGui::Text("Spawning %s", m.c_str());
          ImGui::EndDragDropSource();
        }

        if (ImGui::IsItemHovered())
          ImGui::SetTooltip("%s (arrastrar al Viewport o doble clic)", m.c_str());
        if (ImGui::IsItemHovered() &&
    ...
    if (ImGui::BeginTabItem("Textures")) {
      ...
      for (const AssetThumb &t : textureThumbs) {
        ...
        ImGui::BeginGroup();
        if (t.srv)
          ImGui::Image((ImTextureID)t.srv, ImVec2(cell, cell));
        else
        ...
        if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID)) {
          ...
          ImGui::SetDragDropPayload(
              "DND_TEXTURE_PATH", fullPath.c_str(), fullPath.size() + 1);
    ...
    if (ImGui::BeginTabItem("Audio")) {
      ...
      for (const fs::directory_entry &entry :
        ...
        if (!error && entry.is_regular_file() && IsPlayableAudioFile(entry.path())) {
          sounds.push_back(ToContentPath(entry.path().lexically_relative(audioRoot)));
        }
      }
      if (sounds.empty()) {
      ...
      for (const std::string &sound : sounds) {
        ...
        if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID)) {
          ...
          ImGui::SetDragDropPayload(
              "DND_AUDIO_PATH", fullPath.c_str(), fullPath.size() + 1);
...
void
GUI::drawSelectionOutline(Camera &cam,
                          const EU::Vector3 &mn,
                          const EU::Vector3 &mx,
  ...
  for (int c = 0; c < 8; ++c) {
    ...
    XMVECTOR clip = XMVector4Transform(
        XMVectorSet(
            XMVectorGetX(worldC), XMVectorGetY(worldC), XMVectorGetZ(worldC), 1.0f),
        vp);
