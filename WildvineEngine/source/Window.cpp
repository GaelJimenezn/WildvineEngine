...
Window::init(HINSTANCE hInstance, int nCmdShow, WNDPROC wndproc, BaseApp *app) {
  // Se guarda la instancia de la aplicación (handle de la instancia) en una variable
  // miembro.
  m_hInst = hInstance;

  //
  // Se registra la clase de la ventana. Esto define el "tipo" de ventana que se creará,
  // incluyendo su icono, cursor y procedimiento de ventana (la función que manejará los
  // mensajes).
  //
  WNDCLASSEX wcex = {};
  wcex.cbSize = sizeof(WNDCLASSEX);
  wcex.style =
      CS_HREDRAW | CS_VREDRAW; // La ventana se redibujará si se cambia su tamaño.
  wcex.lpfnWndProc = wndproc;  // Se asigna el procedimiento de ventana.
  wcex.cbClsExtra = 0;
  wcex.cbWndExtra = 0;
  wcex.hInstance = m_hInst;
  wcex.hIcon =
      LoadIcon(m_hInst, (LPCTSTR)IDI_TUTORIAL1); // Se carga el icono de la aplicación.
  wcex.hCursor = LoadCursor(NULL, IDC_ARROW);    // Se asigna el cursor por defecto.
  wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
  wcex.lpszMenuName = NULL;
  wcex.lpszClassName =
      "TutorialWindowClass"; // Se le da un nombre a la clase de la ventana.
  ...
  //
  // Se crea la ventana. Primero, se define el tamaño deseado del "área de cliente" (el
  // área de dibujo), y luego se ajusta para incluir la barra de título y los bordes.
  //
  ...
  m_hWnd = CreateWindow(
      "TutorialWindowClass", // Nombre de la clase de la ventana que se va a crear.
      ...
      app);
  // Si la creación de la ventana falla, se muestra un mensaje de error y se devuelve
  // E_FAIL.
  if (!m_hWnd) {
...
// suele ser gestionada automáticamente por el sistema operativo al salir de la
// aplicación.
//
void
Window::destroy() {
