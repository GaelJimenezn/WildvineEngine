...
//
// La funciï¿½n `destroy` se encarga de liberar el objeto principal de Direct3D, el
// ID3D11DeviceContext. Este contexto es el "motor" que se utiliza para enviar comandos de
// renderizado a la GPU.
//
void
...
//
// `RSSetViewports` configura el(los) viewport(s) de la etapa de rasterizaciï¿½n.
// Un viewport define el ï¿½rea de la ventana de salida donde se renderizarï¿½ la imagen.
//
void
DeviceContext::RSSetViewports(unsigned int NumViewports,
                              const D3D11_VIEWPORT *pViewports) {
  // Verificaciï¿½n para evitar un puntero nulo.
  if (!pViewports) {
    ERROR("DeviceContext", "RSSetViewports", "pViewports is nullptr");
    return;
  }

  // Se llama a la funciï¿½n nativa de Direct3D.
  m_deviceContext->RSSetViewports(NumViewports, pViewports);
}

//
// `PSSetShaderResources` asigna recursos (como texturas) al Pixel Shader.
// La GPU lee estos recursos para aplicar detalles de color y textura a los pï¿½xeles.
//
void
...
//
// `IASetInputLayout` asigna un diseï¿½o de entrada para la etapa de Ensamblador de
// Entrada
// (IA). Este diseï¿½o le dice a la GPU cï¿½mo interpretar el formato de los datos del
// bï¿½fer
// de vï¿½rtices.
//
void
...
// Este sombreador procesa las posiciones de los vï¿½rtices y otros atributos antes de la
// rasterizaciï¿½n.
//
void
...
// Este sombreador determina el color final de cada pï¿½xel en la pantalla.
//
void
...
//
// `UpdateSubresource` actualiza los datos de un recurso (como un bï¿½fer o una textura)
// en
// la GPU. Se utiliza para copiar nuevos datos de la memoria del CPU a la memoria de la
// GPU.
//
void
DeviceContext::UpdateSubresource(ID3D11Resource *pDstResource,
  ...
  // Verificaciï¿½n para evitar punteros nulos.
...
//
// `IASetVertexBuffers` asigna bï¿½feres de vï¿½rtices a la etapa de Ensamblador de
// Entrada.
// Estos bï¿½feres contienen los datos de los vï¿½rtices (posiciones, normales,
// coordenadas de
// textura, etc.).
//
void
...
//
// `IASetIndexBuffer` asigna un bï¿½fer de ï¿½ndices a la etapa de Ensamblador de Entrada.
// Un bï¿½fer de ï¿½ndices especifica el orden en que se usan los vï¿½rtices para formar
// primitivas (triï¿½ngulos, lï¿½neas).
//
void
...
// Los samplers controlan cï¿½mo se "muestrean" o leen los datos de las texturas.
//
void
...
//
// `RSSetState` asigna un estado de rasterizaciï¿½n.
// Esto controla el comportamiento de la etapa de rasterizaciï¿½n, como el modo de relleno
// de polï¿½gonos.
//
void
...
// Esto controla cï¿½mo se mezclan los pï¿½xeles renderizados con los pï¿½xeles que ya
// existen
// en el render target.
//
void
...
//
// `OMSetRenderTargets` asigna uno o mï¿½s render targets y un buffer de
// profundidad/plantilla. Esto establece dï¿½nde la GPU debe renderizar el resultado de la
// escena.
//
void
...
//
// `IASetPrimitiveTopology` establece el tipo de primitivas (triï¿½ngulos, puntos,
// lï¿½neas)
// que se dibujarï¿½n.
//
void
DeviceContext::IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY Topology) {
  // Verificaciï¿½n para evitar un tipo de topologï¿½a indefinido.
...
//
// `ClearRenderTargetView` limpia el render target con un color sï¿½lido.
// Esto se hace tï¿½picamente al comienzo de cada fotograma para borrar el contenido
// anterior.
//
void
...
//
// `VSSetConstantBuffers` asigna bï¿½feres de constantes al Vertex Shader.
// Los bï¿½feres de constantes se utilizan para pasar datos de la CPU (como matrices de
// transformaciï¿½n) a los sombreadores.
//
void
...
//
// `PSSetConstantBuffers` asigna bï¿½feres de constantes al Pixel Shader.
// Se usa para pasar datos como la posiciï¿½n de las luces, el color del material, etc.
//
void
...
//
// `DrawIndexed` es la funciï¿½n de dibujo principal.
// Le dice a la GPU que dibuje primitivas usando los bï¿½feres de vï¿½rtices e ï¿½ndices
...
DeviceContext::DrawIndexed(unsigned int IndexCount,
  ...
  // Verificaciï¿½n para evitar un dibujo sin ï¿½ndices.
  if (IndexCount == 0) {
