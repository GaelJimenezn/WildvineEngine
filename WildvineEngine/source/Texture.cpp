...
bool
IsTextureCacheUpToDate(const std::string &sourcePath, const std::string &cachePath) {
...
bool
LoadTextureCache(const std::string &cachePath, CachedTextureData &outTexture) {
  ...
  if (!stream.good() || magic != kTextureCacheMagic || version != kTextureCacheVersion ||
      outTexture.width <= 0 || outTexture.height <= 0 ||
...
HRESULT
InitTextureFromImage(Device &device, const std::string &fullPath, Texture &texture) {
  ...
  HRESULT hr = CreateTextureFromRGBA(
      device, width, height, uploadData, &texture.m_texture, &texture.m_textureFromImg);
  if (decodedData) {
...
Texture::initFromMemory(Device &device,
  ...
  if (!decodedData) {
    ERROR("Texture",
          "initFromMemory",
          ("Failed to load texture from memory: " + std::string(stbi_failure_reason()))
              .c_str());
    return E_FAIL;
  }

  HRESULT hr = CreateTextureFromRGBA(
      device, width, height, decodedData, &m_texture, &m_textureFromImg);
  stbi_image_free(decodedData);
...
Texture::initSingleChannelFromMemory(Device &device,
  ...
  HRESULT hr = CreateTextureFromRGBA(
      device, width, height, channelData.data(), &m_texture, &m_textureFromImg);
  stbi_image_free(decodedData);
...
Texture::initSolidColor(Device &device,
  ...
  HRESULT hr = CreateTextureFromRGBA(device, 1, 1, pixel, &m_texture, &m_textureFromImg);
  if (FAILED(hr)) {
    SAFE_RELEASE(m_texture);
    SAFE_RELEASE(m_textureFromImg);
...
HRESULT
Texture::initFromFile(Device &device, const std::string &fullPath) {
  if (!device.m_device) {
  ...
  HRESULT hr = S_OK;
  if (lower.size() >= 4 && lower.compare(lower.size() - 4, 4, ".dds") == 0) {
    hr = D3DX11CreateShaderResourceViewFromFile(device.m_device,
                                                resolvedPath.c_str(),
                                                nullptr,
                                                nullptr,
...
Texture::init(Device &device,
  ...
  if (FAILED(hr)) {
    ERROR(
        "Texture",
        "init",
        ("Failed to create texture with specified params. HRESULT: " + std::to_string(hr))
            .c_str());
    return hr;
  }
...
Texture::CreateCubemap(Device &device,
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    if (i == 0) {
      ...
      ERROR(
          "Texture", "CreateCubemap", "All cubemap faces must have the same dimensions.");
      for (int k = 0; k <= i; ++k) {
        if (facePixels[k]) {
          stbi_image_free(facePixels[k]);
        }
  ...
  if (!generateMips) {
    ...
    for (UINT face = 0; face < 6; ++face) {
      ...
      deviceContext.UpdateSubresource(
          m_texture, sub, nullptr, facePixels[face], width * 4, 0);
    }
  }
