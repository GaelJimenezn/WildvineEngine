...
// Aquí se crea la vista de Direct3D que nos permite usar la textura como un buffer de
// profundidad.
//
HRESULT
DepthStencilView::init(Device &device, Texture &depthStencil, DXGI_FORMAT format) {
  ...
  HRESULT hr = device.m_device->CreateDepthStencilView(
      depthStencil.m_texture, &descDSV, &m_depthStencilView);
...
// Esto es necesario al comienzo de cada fotograma para que la información del anterior no
// interfiera.
//
void
DepthStencilView::render(DeviceContext &deviceContext) {
  //
  // Verificación de errores: se asegura de que el contexto del dispositivo y la vista no
  // sean nulos.
  //
  ...
  // Se establecen los valores por defecto: 1.0f para profundidad (el punto más lejano) y
  // 0 para plantilla.
  //
...
//
// La función `destroy` libera la memoria de la vista de profundidad/plantilla cuando ya
// no es necesaria. Se usa la macro SAFE_RELEASE para asegurar que se libere
// correctamente.
//
void
