...
Skybox::init(Device &device, DeviceContext *deviceContext, Texture &cubemap) {
  ...
  const unsigned int indices[] = {// back (-Z)
                                  0,
                                  1,
                                  2,
                                  0,
  ...
  if (FAILED(hr)) {
    ERROR("Skybox",
          "init",
          ("Failed to initialize NeverChanges Buffer. HRESULT: " + std::to_string(hr))
              .c_str());
    return hr;
  }
...
Skybox::render(DeviceContext &deviceContext) {
  ...
  if (!m_cubeModel || !m_skyboxTexture.m_textureFromImg)
    return;
